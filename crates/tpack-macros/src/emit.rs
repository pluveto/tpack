...
impl<'a> Emitter<'a> {
    ...
    fn impl_serialize(&self) -> Result<TokenStream, String> {
        match &self.item.kind {
            ItemKind::Struct(fields) => Self::impl_struct_serialize(&self.item.name, fields),
            ItemKind::Enum(variants) => Self::impl_enum_serialize(&self.item.name, variants),
        }
    }

    fn impl_deserialize(&self) -> Result<TokenStream, String> {
        match &self.item.kind {
            ItemKind::Struct(fields) => Self::impl_struct_deserialize(&self.item.name, fields),
            ItemKind::Enum(variants) => Self::impl_enum_deserialize(&self.item.name, variants),
        }
    }
    ...
    fn impl_struct_deserialize(name: &str, fields: &[Field]) -> Result<TokenStream, String> {
        ...
        let declarations = fields
            ...
            .enumerate()
            .map(|(index, _)| {
                format!("let mut __tpack_field_{index} = ::std::option::Option::None;")
            })
            .collect::<Vec<_>>()
            .join("\n");
    ...
    fn impl_enum_serialize(name: &str, variants: &[EnumVariant]) -> Result<TokenStream, String> {
        if variants.iter().all(|variant| variant.payload_ty.is_none()) {
            ...
            let arms = variants
                ...
                .collect::<Vec<_>>()
                .join(", ");
            return Self::parse_tokens(format!(
                r#"
                impl ::tpack::TpackSerialize for {name} {{
                    fn schema() -> ::tpack::Schema {{
